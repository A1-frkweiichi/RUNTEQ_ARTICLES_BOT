...
class GoogleSheetsService
  ...
  def record_post(params)
    ...
    japanese_day = japanese_day_of_week(current_time.wday)
    formatted_time = current_time.strftime('%H:%M')

    full_date_time = "#{formatted_date}(#{japanese_day}) #{formatted_time}"

    values = [
      [params[:article_title], params[:article_url], x_username_url, cleaned_hashtag, full_date_time]
    ]
